...
/** C-PR-07 — all notices, including institution-level read receipts. */
export function PrincipalNoticesPage() {
  const [query, setQuery] = useState("");
  const [scope, setScope] = useState<"" | "INSTITUTION" | "DEPARTMENT" | "CLASS">("");
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const resource = useResource(
    () => fetchPrincipalNotices({ query: query || undefined, scope: scope || undefined, limit: 100 }),
    [query, scope],
  );
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <PageHeader
        ...
        action={<Link href="/principal/notices/new" className="inline-flex h-10 items-center gap-2 rounded-field bg-accent px-4 text-sm font-semibold text-white shadow-accent transition hover:bg-accent-hover"><Plus className="h-4 w-4" /> Post notice</Link>}
      />
      <Card className="mb-5 !p-4">
        <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_220px]">
          ...
          <div><label htmlFor="notice-scope" className="sr-only">Filter scope</label><select id="notice-scope" value={scope} onChange={(event) => setScope(event.target.value as typeof scope)} className={inputClass}><option value="">All scopes</option><option value="INSTITUTION">Institution-wide</option><option value="DEPARTMENT">Department</option><option value="CLASS">Class</option></select></div>
        </div>
      </Card>
      <AsyncState loading={resource.loading} error={resource.error} onRetry={resource.reload} loadingLabel="Loading notices…">
        {resource.data ? resource.data.items.length ? <div className="space-y-3">{resource.data.items.map((notice) => <NoticeCard key={notice.id} notice={notice} onOpen={() => setSelectedId(notice.id)} />)}</div> : <EmptyState text="No notices match this filter." /> : null}
      </AsyncState>
...
function NoticeCard({ notice, onOpen }: { notice: PrincipalNoticeRow; onOpen: () => void }) {
  ...
  return (
    <Card className={notice.is_pinned ? "border-accent-border" : ""}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button type="button" onClick={onOpen} className="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-field border border-border px-3 text-xs font-semibold text-foreground transition hover:border-accent hover:text-accent"><Eye className="h-3.5 w-3.5" /> {notice.read_count} read</button>
      </div>
    </Card>
  );
}
...
function NoticeDetail({ id, onClose }: { id: string; onClose: () => void }) {
  const resource = useResource(() => fetchPrincipalNotice(id), [id]);
  return (
    <div role="dialog" aria-modal="true" aria-label="Notice details" className="fixed inset-0 z-50 flex items-center justify-center bg-primary/50 p-4">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-card bg-white p-5 shadow-2xl sm:p-6">
        ...
        <AsyncState loading={resource.loading} error={resource.error} onRetry={resource.reload} loadingLabel="Loading notice…">
          {resource.data ? <NoticeDetailContent notice={resource.data} /> : null}
        </AsyncState>
      </div>
    </div>
  );
...
function NoticeDetailContent({ notice }: { notice: PrincipalNoticeDetail }) {
  return (
    <div>
      ...
      <h4 className="border-t border-border pt-5 font-display text-base font-bold text-primary">Read receipts ({notice.readers.length})</h4>
      {notice.readers.length ? <ul className="mt-3 divide-y divide-border rounded-field border border-border">{notice.readers.map((reader) => <li key={reader.id} className="flex items-center justify-between gap-3 px-3 py-3 text-sm"><span className="font-medium text-primary">{reader.name}</span><time className="text-xs text-muted-foreground">{dateTime(reader.read_at)}</time></li>)}</ul> : <p className="mt-3 text-sm text-muted-foreground">No recipients have read this notice yet.</p>}
    </div>
  );
}

/** C-PR-08 — compose a tenant-validated institution, department or class notice. */
export function PrincipalNoticeComposerPage() {
  const router = useRouter();
  const targets = useResource(fetchPrincipalNoticeTargets, []);
  const [form, setForm] = useState({ title: "", body: "", targetScope: "INSTITUTION" as "INSTITUTION" | "DEPARTMENT" | "CLASS", targetId: "", priority: "NORMAL" as "NORMAL" | "IMPORTANT" | "URGENT", pinned: false, expiresAt: "" });
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader title="Post notice" subtitle="Publish an institution-wide, department or class notice. Recipient scope is verified by the server." />
      <AsyncState loading={targets.loading} error={targets.error} onRetry={targets.reload} loadingLabel="Loading notice targets…">
        <Card>
          <form onSubmit={submit} className="space-y-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2"><div><label htmlFor="notice-target-scope" className={labelClass}>Audience</label><select id="notice-target-scope" className={inputClass} value={form.targetScope} onChange={(event) => setForm({ ...form, targetScope: event.target.value as typeof form.targetScope, targetId: "" })}><option value="INSTITUTION">Institution-wide</option><option value="DEPARTMENT">Department</option><option value="CLASS">Class</option></select></div><div><label htmlFor="notice-priority" className={labelClass}>Priority</label><select id="notice-priority" className={inputClass} value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value as typeof form.priority })}><option value="NORMAL">Normal</option><option value="IMPORTANT">Important</option><option value="URGENT">Urgent</option></select></div></div>
