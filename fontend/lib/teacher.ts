/**
 * Teacher console API client (C-TC-01 … C-TC-22).
 *
...
const call = <T>(path: string, init: RequestInit = {}): Promise<T> =>
  leadershipCall<T>(API_PREFIX, path, init, "TeacherAPIError");
...
export type TeacherPage<T> = PrincipalPage<T>;
...
export const fetchTeachingAssignments = () =>
  call<TeachingAssignment[]>("/teaching-assignments");
...
export const fetchTeacherExam = (examId: string) =>
  call<TeacherExamDetail>(`/examinations/${examId}`);
...
export type TeacherAssignmentUpdate = Partial<Omit<TeacherAssignmentCreate, "subject_id" | "class_id" | "publish">>;
...
export const fetchTeacherDiscussion = (filters: {
  query?: string;
