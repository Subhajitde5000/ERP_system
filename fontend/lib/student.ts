/**
 * Student console API client (C-ST-01 … C-ST-20).
 *
...
const call = <T>(path: string, init: RequestInit = {}): Promise<T> =>
  leadershipCall<T>(API_PREFIX, path, init, "StudentAPIError");
...
export type StudentPage<T> = PrincipalPage<T>;
...
/** C-ST-08 anti-cheat: report that the student left the exam tab. */
