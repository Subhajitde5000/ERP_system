/**
 * Parent portal API client (C-PA-01 … C-PA-12) — the mobile port of
 * `fontend/lib/parent.ts`. Same endpoints, same snake_case wire format, the same
 * shapes as `backend/app/schemas/parent.py`; only the transport differs, because the
 * app holds one tenant at a time (the slug is typed at login) and has no subdomain to
 * guard.
 *
 * The caller is a guardian, not an operator: every `/children/{id}/…` call is
 * re-checked on the server against that child's `parent_student_links` row, so this
 * file carries no "is X visible" helpers — hiding a tab is a courtesy to the parent,
 * the 403 is what protects the student.
 */

import { APIError, requestJson } from "./api-client";
import { API_BASE_URL, getAccessToken, refreshAccessToken } from "./auth";
import { queryString } from "./student";
...
const call = <T>(path: string, init: RequestInit = {}): Promise<T> =>
  requestJson<T>(
    `${API_BASE_URL}/api/v1/parent${path}`,
    init,
    getAccessToken(),
    "ParentAPIError",
    refreshAccessToken,
